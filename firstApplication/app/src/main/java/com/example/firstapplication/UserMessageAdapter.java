...
import com.example.firstapplication.model.UserMessage;
...
public class UserMessageAdapter extends RecyclerView.Adapter<UserMessageAdapter.UserMessageViewHolder> {
    ...
    public UserMessageViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == UserMessage.TYPE_RECEIVE) {
            //用户接收的消息，显示在左侧
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.get, parent, false);
            //    Log.v(GameSystem.LOG_TAG, "left view.toString():" + view.toString());
            return new UserMessageViewHolder(view);
        } else {
            //用户发送的消息，显示在右侧
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.send, parent, false);
            return new UserMessageViewHolder(view);
        }
    ...
    class UserMessageViewHolder extends RecyclerView.ViewHolder {
        ...
        UserMessageViewHolder(@NonNull View itemView) {
            ...
            tvLeft = itemView.findViewById(R.id.getText);
            tvRight = itemView.findViewById(R.id.sendText);
